...
#
#   bash tests/e2e/fork.sh
#   FORK_RPC=https://… bash tests/e2e/fork.sh   (skips the upstream probe)
#
...
SCHEMA_REGISTRAR=$(jq -r .schema_registrar tests/e2e/deploy.json)
...
REGISTRY=$(forge create contracts/src/registry/InstanceRegistry.sol:InstanceRegistry \
  ...
  | jq -r .deployedTo)
USDC=$(forge create contracts/src/tokens/TestUSDC.sol:TestUSDC \
  --rpc-url "$RPC" --private-key "$PK" --broadcast --json | jq -r .deployedTo)
FEED=$(forge create contracts/test/mocks/MockEthUsdFeed.sol:MockEthUsdFeed \
  --rpc-url "$RPC" --private-key "$PK" --broadcast --json | jq -r .deployedTo)
NOW=$(cast block latest --field timestamp --rpc-url "$RPC")
cast send "$FEED" "set(int256,uint256)" 300000000000 "$NOW" \
  --rpc-url "$RPC" --private-key "$PK" >/dev/null   # $3000.00000000, 8dp

VAULT=$(forge create contracts/src/vault/ProvingVault.sol:ProvingVault \
...
MOCK_GATEWAY=$(forge create contracts/test/mocks/MockSP1Gateway.sol:MockSP1Gateway \
  --rpc-url "$RPC" --private-key "$PK" --broadcast --json | jq -r .deployedTo)
...
SNAP_DEPLOYER=$(forge create contracts/src/factory/InstanceDeployers.sol:MerkleSnapshotDeployer \
  --rpc-url "$RPC" --private-key "$PK" --broadcast --json | jq -r .deployedTo)
DIST_DEPLOYER=$(forge create contracts/src/factory/InstanceDeployers.sol:MerkleFundDistributorDeployer \
  --rpc-url "$RPC" --private-key "$PK" --broadcast --json | jq -r .deployedTo)
PARAMS_DEPLOYER=$(forge create contracts/src/factory/InstanceDeployers.sol:TrustgraphsParamsControllerDeployer \
  --rpc-url "$RPC" --private-key "$PK" --broadcast --json | jq -r .deployedTo)
FACTORY=$(forge create contracts/src/factory/TrustgraphsFactory.sol:TrustgraphsFactory \
  --rpc-url "$RPC" --private-key "$PK" --broadcast --json \
...
forge script contracts/script/examples/CreateInstance.s.sol:CreateInstance \
...
attest_curated() { # the curated instance has its own resolver, so its own schema
  forge script contracts/script/E2eAttest.s.sol:E2eAttest --sig "run(address,bytes32)" "$EAS" "$CUR_SCHEMA" \
    --rpc-url "$RPC" --private-key "$1" --broadcast --skip-simulation >>"$WORK/attest.log" 2>&1
}
...
BOGUS=$(forge create contracts/src/merkle/SP1JournalVerifier.sol:SP1JournalVerifier \
  --rpc-url "$RPC" --private-key "$PK" --broadcast --json \
