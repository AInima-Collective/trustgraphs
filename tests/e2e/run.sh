...
PARAMS_HASH=$(jq -r .params_hash tests/e2e/deploy.json)
...
if [ "${E2E_ONCHAIN:-1}" = "1" ]; then
  ...
  SIGNER_GATEWAY=$(forge create contracts/test/mocks/MockSP1Gateway.sol:MockSP1Gateway \
    --rpc-url "$RPC" --private-key "$PK" --broadcast --json | jq -r .deployedTo)
  cast send "$SIGNER_GATEWAY" "setExpectedVKey(bytes32)" "$SIGNER_VKEY" --rpc-url "$RPC" --private-key "$PK" >/dev/null
  SIGNER_VERIFIER=$(forge create contracts/src/merkle/SP1JournalVerifier.sol:SP1JournalVerifier \
    --rpc-url "$RPC" --private-key "$PK" --broadcast --json \
  ...
  HC_REGISTRY=$(forge create contracts/src/registry/AnchorRegistry.sol:AnchorRegistry \
    --rpc-url "$RPC" --private-key "$PK" --broadcast --json \
    --constructor-args "$DEPLOYER" 200000 | jq -r .deployedTo)
  HC_GATEWAY=$(forge create contracts/test/mocks/MockSP1Gateway.sol:MockSP1Gateway \
    --rpc-url "$RPC" --private-key "$PK" --broadcast --json | jq -r .deployedTo)
  ...
  HC_TWIN_ACC=$(forge create contracts/src/merkle/EmptyLaneAccumulator.sol:EmptyLaneAccumulator \
    --rpc-url "$RPC" --private-key "$PK" --broadcast --json | jq -r .deployedTo)
  HC_TWIN=$(forge create contracts/src/merkle/MerkleSnapshot.sol:MerkleSnapshot \
    --rpc-url "$RPC" --private-key "$PK" --broadcast --json \
  ...
  IREG=$(forge create contracts/src/registry/InstanceRegistry.sol:InstanceRegistry \
    --rpc-url "$RPC" --private-key "$PK" --broadcast --json \
