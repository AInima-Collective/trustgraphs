...
Unlike the `tests/fixtures/atproto/repos/` snapshots (real Bluesky repos pulled from the relay),
...
- `tests/fixtures/atproto/hypercerts/walk` — the MST/commit/parity/EIP-712 walker (see below).
- `crates/envelopes/tests/atproto_real.rs` — `hypercerts_seeded_repo_verifies_all_collections`
  (pins alice's record count = 6).
- `crates/hypercerts-core/src/records.rs` — `all_fixture_records_decode` (count = 6, field values).
- `crates/hypercerts-core/src/binding.rs` — `fixture_binding_recovers` (alice DID + EVM address).
- `crates/hypercerts-core/tests/compute_fixture.rs` — single-repo pipeline (alice DID/BOB DID/address).
- `crates/hypercerts-core/tests/two_sided_fixture.rs` — **multi-repo** pipeline over BOTH
