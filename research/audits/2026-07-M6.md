...
1. `contracts/src/eas/resolvers/ContributionResolver.sol`
2. `contracts/src/merkle/TrustAccumulatorMirror.sol`
3. `contracts/src/merkle/MerkleFundDistributor.sol` — M6 delta only (commit `4a41f6b`)
4. `contracts/src/params/ContributionsParamsCodec.sol`
5. `contracts/script/DeployContributionsInstance.s.sol` + `contracts/script/lib/ContributionsParamsJson.sol`
6. `contracts/src/tokens/TestUSDC.sol`
...
| M6-7 | INFO | ContributionsParamsCodec | **Reviewed clean.** Byte-identical to `contributions_core::params::params_hash`: all 21 fields in the same order (`src/lib.rs` struct ↔ codec ↔ `params.rs`), and Solidity `abi.encode` of static types == the Rust right-aligned big-endian words (`crates/zk-core/src/words.rs`). Golden-locked by `ContributionsGoldenVectors.t.sol`. Schema UIDs (slots 19–21) are supplied by the deploy, not the params file, so `paramsHash` binds the freshly registered UIDs in one pass. | Word-by-word comparison. | No finding. |
...
[`research/operations/contributions/runbook.md`](../../research/operations/contributions/runbook.md).
