...
- **Nostr events are structurally circuit-friendly, now on the corrected measured path.** An event id *is* the SHA-256 of a canonical serialization (NIP-01), and the signature is BIP-340 schnorr over secp256k1 — the curve our patched guest stack already runs. The guest never parses attacker JSON: it re-serializes from a structured witness and hashes. No CBOR, no MST, no DID directory. The corrected prehash-compatible run measures 44,734 cycles / 70,149 PGU per complete event and 3,631,054 PGU for the 35-event live envelope; the 2026-08-16 message-API values remain retired. The ground is still unoccupied: no public project proving Nostr events inside a zkVM was found — the offchain research's "nobody ZK-proves graph compute over offchain attestations" claim extends cleanly to Nostr.
...
Module: `crates/envelopes/src/nostr/` behind the same loose trait as its siblings ("given a head and witness bytes, either produce the complete, authenticated event set behind this head, or fail"). Mirroring the atproto module's six-step `verify()`:
...
- **BIP-340 rides the existing patched `k256` fork, through its prehash API.** The `patch-k256-13.4-sp1-*` diff patches the `schnorr` module explicitly (verify = tagged sha256 challenge + `lincomb` over the secp256k1 precompiles; x-only key load rides hint-accelerated field `sqrt`), and Succinct's own patch-testing suite proves a schnorr guest in CI. Correction (2026-08-19): the earlier 31,747-cycle / 56,521-PGU row called message-level `Verifier`, which hashes the already-hashed Nostr event id; it is retired. The corrected source-derived run uses `PrehashVerifier` and measures 30,842 cycles / 55,182 PGU per signature. The sp1-patches C `secp256k1` fork still does *not* accelerate schnorr, so rust-nostr's own `Event::verify` remains host-side-only; the guest uses patched `sha2` + `k256::schnorr::hazmat::PrehashVerifier` directly.
...
Reuse the hypercerts binding machinery verbatim (`crates/hypercerts-core/src/binding.rs`; digest derivation pinned three-way in [`offchain/05-spike-results.md`](./offchain/05-spike-results.md) §2.4):
...
3. **In-guest verification:** the envelope's corrected prehash-path schnorr verification is the npub-side consent — the same key that authors edges signed the carrier. Then recompute the EIP-712 digest → ecrecover (the still-valid marginal reference is 27.3k cycles / 50.2k PGU) → recovered address must equal both `message.evmAddress` and the event's claimed address; `message.did` must equal the did:nostr string of the carrier event's **author** pubkey; `chainId` must match the instance's chain. These are exactly the binding.rs checks with "record lives in the DID's repo" replaced by "event signed by the npub." The event portion is now measured at 44.7k cycles / 70.1k PGU; the production S2 binding row will add the EIP-712 field checks rather than recharging another Nostr signature.
...
## 8. Cost model (corrected and measured 2026-08-19)

On SP1 6.3.1, the corrected marginal rows are 30,842 cycles / 55,182 PGU for BIP-340 prehash verification, 44,734 / 70,149 for a complete NIP-01 event, 10,302 / 11,920 for one Buzz audit fold, and 35,964 / 61,246 for one NIP-OA credential. The source-derived 20,297-byte live Option-A bundle—30 audit rows, 35 signed events, and three OA checks—executes end to end in 2,519,703 cycles / 3,631,054 PGU. Exact inputs and raw rows are checked in under [`nostr/`](./nostr/).

The conservative operator formula is `2 × (24×bundle_bytes + 12,000×audit_entries + 71,000×NIP-01_checks + 62,000×OA_checks + 1,000,000)` PGU. It double-charges bundle bytes at the measured SHA-256 rate, then doubles the entire crypto/hash subtotal for decode, coverage, state rules, input shape, and variance. It estimates the live run at 9.04M PGU versus 3.63M actual. Joint hard maxima—12 MiB, 4,096 audit entries, 640 NIP-01 checks, and 256 OA checks—estimate to 826.9M PGU, below the 1B-PGU default. The pilot ceiling is 4 MiB, 2,048 audit entries, 512 unique events / 640 total NIP-01 checks, 128 OA checks, and 400M estimated PGU.

No dollar projection is frozen without a current prover-network clearing price. Re-measure on any patch-tag or SP1 major bump: Schnorr acceleration is source-evidenced and upstream-CI-tested, but not advertised in Succinct's public feature table.
...
**S0 — verification spike (exited 2026-08-19).** (1) The corrected `PrehashVerifier` marginal rows and whole-live-TGNW guest run are checked in, and their conservative model freezes the 12-MiB hard / 400M-PGU pilot ceilings; the old message-API numbers remain retired. (2) The patched pinned Buzz profile produced 35 signed DB rows, a gap-free 30-row audit prefix, 5 explicitly identified direct rows, a canonical 20,297-byte live TGNW projection, and an independent offline round trip. Serializer byte parity includes every ASCII control byte, but U+0000 is correctly serializer-only because PostgreSQL `jsonb` rejects its `\u0000` tag escape before storage. (3) The Option-A source question is answered (`EventCreated` covers ordinary stored events, enqueue-before-OK; roster and four generated notification rows in the observed path are direct-row exceptions), and the live export proves audit-enabled full coverage. (4) J1, binding/head kinds, limits, and replacement/deletion behavior are frozen in the pinned fixture ADR. (5) Six independent tests cover every binary field mutation, exact/first-rejected caps, and the hard/pilot work formulas. S1 now owns the reusable production verifier and conformance guest.

**S1 — envelope module + conformance guest (exited 2026-08-19).** `crates/envelopes/src/nostr/` now supplies strict canonical TGNW A/C decoding, node ids, exact Buzz and self-log head folds, NIP-01 and NIP-OA verification, relay roster eligibility, replacement/deletion, and closed accepted/skipped outcome digests. The detached `zk/nostr-program/` guest commits `(nodeId, head, count, dataCommitment, acceptedEventsDigest, skippedDigest)` without rotating any shipped production vkey. The live fixture agrees byte-for-byte natively and in SP1 at 3,501,450 cycles / 4,553,481 PGU; both paths reject a re-committed signed-byte mutation. Fourteen native tests cover A/C positives, every authenticated Option-A field class, signed adversarial OA/deletion vectors, missing audited events, replacement order independence, exact caps, and bounded decoder no-panic/canonical behavior. The fixture's missing-reference and A+C duplicate cases intentionally remain S2 graph-semantic tests.

**S2 — the nostr-workspace program.** `crates/nostr-workspace-core`: edge rules V1/G1/J1/F1 (§12.3), params + `params_hash`, journal v3, `compute()` on the generic rank/distribute machinery; golden vectors 4-way (`tests/golden/nostr-workspace.json`), `NostrWorkspaceParamsCodec.sol` + golden test, TS recompute port. The platform claim says this touches zero pagerank-core semantic lines — hypercerts proved it.
