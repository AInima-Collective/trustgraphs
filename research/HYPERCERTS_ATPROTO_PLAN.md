...
**Status:** Realized, 2026-07-14 plan — the hypercerts program is **Built** (the build plan's M1–M4 are exit-green; the M5 pilot deploy is pending). Current status: [`docs/PROGRAMS.md`](../docs/PROGRAMS.md); deviations of record: [`docs/DEVIATIONS.md`](../docs/DEVIATIONS.md).
...
## 8. Cost model (updated 2026-07-14 from M1 measurements — [`offchain/05-spike-results.md`](./offchain/05-spike-results.md))

Measured on SP1 6.3.1 (patched crates, marginal per-op, PGU = `ExecutionReport.gas`): k256 ecrecover **27.3k cycles / 50.2k PGU per sig** (the pre-spike ~220k-cycle anchor was a v1-era figure — **8× too high**); p256 verify **104.7k cycles / 113.1k PGU** (3.8× k256 — self-hosted-PDS p256 commits cost more, still trivial); SHA-256 **219 cycles / 732 PGU per 64-B block** (PGU/cycle 3.35 — precompiled hashing under-reports prover cost in cycles; sha256 is cheaper per byte than keccak, 11.4 vs 15.2 PGU/B).

Launch-scale 1k repos / 50–100k records: commit sigs 1k × 50k ≈ **0.05B PGU**; MST+record SHA-256 (~8 blocks/record) ≈ **0.3–0.6B PGU**; dag-cbor decode still the *(soft)* term — 5–20k cycles/record ≈ 0.5–2B PGU at ~1 PGU/cycle for plain RISC-V (in-guest parser cycle count is the one number deliberately deferred to the M3 parser freeze, §1.8 of the spike doc). Total ≈ **1–2.7B PGU**, decode-dominated — signature and hashing costs are now noise. Dollar figure stays *(soft)* pending the one live prover-network clearing-price observation (blocked on `NETWORK_PRIVATE_KEY`); at dossier-03's assumed rates it remains **single-digit dollars and well under an hour**, monolithic (aggregation threshold ~120B PGU is far away). At 10k repos, per-repo compressed sub-proofs (cached until `rev` changes) move cost to churn — platform Phase D, not launch.
