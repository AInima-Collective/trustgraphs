...
> Internal verification reference retained from the original public guide.
...
- **The scoring parameters are pinned by hash.** The proof is only valid under the
  ...
  compute with different parameters. See the [algorithm spec](../../docs/concepts/algorithm.md) for
...
[`build/setup.md`](../../docs/build/setup.md) and [`build/quickstart.md`](../../docs/build/quickstart.md). The
...
The flagship [trust-graph program](./trust-graph/runbook.md) ingests EAS attestations
...
[trust-graph runbook](./trust-graph/runbook.md); the unattended version of the loop is
[run a prover](./run-a-prover.md).

## Worked example: reproducing a hypercerts epoch (clean-room, lane 2)

The [hypercerts program](./hypercerts/runbook.md) is the fully-worked example for
...
- [Golden vectors](../../docs/verify/golden-vectors.md): how every port of the algorithm is pinned to the
  proven guest.
