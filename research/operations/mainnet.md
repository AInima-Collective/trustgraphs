...
remediated in `v0.1.0` and later. What the review left as _decisions_, not defects, is carried in
...
- **Region.** Everything in one region. The row asked for us-west2; on the first apply
  (2026-09-14) Railway placed Postgres and the operator volume in us-east4 regardless, so the row
  now says us-east4 for all three and the stateless indexer was moved to match. The Sepolia split
  is a historical accident the IaC comments already describe.
