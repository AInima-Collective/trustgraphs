...
**Implication for trustgraphs:** you cannot rely on atproto for input *history* or non-equivocation. The pattern that works: an indexer/oracle observes commits (firehose or polling), pins `(did, commit CID, rev, MST root)` tuples into your own accumulator (on-chain or committee-signed), and archives the corresponding CAR blocks — because the PDS may not be able to re-serve that commit later. Attestation *revocation* comes for free (delete the record; next observed commit's collection-range proof simply won't contain it), which is arguably a feature for a trust graph, but scores must be defined "as of snapshot set S," not "over all history."
