...
contract TrustComposeParamsController is ITrustComposeParamsController, Ownable2Step {
    ...
    CompositionSourceAccumulator public immutable accumulator;
    ...
    TrustComposeParamsCodec.Params private _initialParams;
    TrustComposeParamsCodec.Params private _currentParams;
    ...
    constructor(
        bytes32 instanceId_,
        address snapshot_,
        CompositionSourceAccumulator accumulator_,
        IInstanceRegistry registry_,
        TrustComposeParamsCodec.Params memory initialParams,
        ...
        bytes32 encoded = TrustComposeParamsCodec.hash(initialParams);
        ...
        returns (uint64 pendingVersion, bytes32 proposalId, uint48 readyAt)
    {
        ...
        TrustComposeParamsCodec.Params memory next = _currentParams;
        ...
        TrustComposeValidator.validateRotation(next, _initialParams);
        bytes32 nextHash = TrustComposeParamsCodec.hash(next);
        ...
        returns (bytes32 newHash)
    {
        ...
        next.policyManifestSha256 = policy.manifestSha256;
        TrustComposeValidator.validateRotation(next, _initialParams);
        newHash = TrustComposeParamsCodec.hash(next);
