...
contract CompositionSourceAccumulator is IAttestationAccumulator {
    ...
    /// @notice Validate the canonical TGCP bytes and exact adapter bindings without changing state.
    ...
    function installPolicy(uint64 version, bytes calldata manifest, address[] calldata adapters) external {
        ...
        TrustComposeValidator.Commitment memory commitment = _validatePolicy(manifest, adapters);
