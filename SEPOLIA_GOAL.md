...
> The operator's *packaging* is a separate program: [GOAL.md](GOAL.md). Its hostable gate
> blocks this milestone, because everything below assumes the daemon can be run somewhere
> other than a developer's checkout. The items here are configuration, not engineering.
