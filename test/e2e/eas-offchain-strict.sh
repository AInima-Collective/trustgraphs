...
FRONTEND_CONFIG_FILE="$(pwd)/frontend/config.development.json"
FRONTEND_CONTRACTS_FILE="$(pwd)/frontend/lib/contracts.ts"
FRONTEND_ABIS_FILE="$(pwd)/frontend/lib/contract-abis.ts"
...
if [ "$BROWSER_MODE" = 1 ]; then
  ...
  pnpm --dir frontend exec playwright --version >/dev/null 2>&1 \
...
FUNDED_KEY="$DEPLOYER_KEY" EAS_OFFCHAIN_E2E_DEPLOY_FILE="$DEPLOY_FILE" \
  forge script script/DeployEasOffchainE2E.s.sol:DeployEasOffchainE2E \
...
for offset in 1 2 3 4; do
  ...
  KUBO_PORTS+=("$port")
  PORT="$port" node test/e2e/eas-offchain-kubo.mjs >"$WORK/kubo-$offset.log" 2>&1 &
  PIDS+=("$!")
  KUBO_PIDS+=("$!")
done
...
if [ "${EAS_OFFCHAIN_E2E_INDEXER:-0}" = 1 ]; then
  ...
  env DATABASE_URL="$DATABASE_URL" DEPLOY_ENV=DEV RPC_URL="$RPC" \
    ...
    pnpm --dir indexer start >"$WORK/indexer.log" 2>&1 &
  INDEXER_PID=$!
  PIDS+=("$INDEXER_PID")
...
start_relays() {
  ...
  env RPC_URL="$RPC" CHAIN_ID="$CHAIN_ID" REGISTRY_ADDRESS="$target_registry" EAS_ADDRESS="$EAS" \
    ...
    ./node_modules/.bin/tsx services/eas-offchain-relay/src/main.ts \
...
if [ "$BROWSER_MODE" = 1 ]; then
  ...
  FRONTEND_DIST_PATH="$(pwd)/frontend/$FRONTEND_DIST_NAME"
  FRONTEND_TSCONFIG_NAME=".tsconfig-eas-offchain-e2e-$$.json"
  FRONTEND_TSCONFIG_PATH="$(pwd)/frontend/$FRONTEND_TSCONFIG_NAME"
  ...
  pnpm --dir frontend wagmi:generate >"$WORK/frontend-wagmi.log" 2>&1 \
    || die "frontend contract address generation failed: $(tail -80 "$WORK/frontend-wagmi.log")"
  cp frontend/tsconfig.json "$FRONTEND_TSCONFIG_PATH"
  ...
  env NEXT_TELEMETRY_DISABLED=1 NEXT_DIST_DIR="$FRONTEND_DIST_NAME" \
    ...
    pnpm --dir frontend exec next build >"$WORK/frontend-build.log" 2>&1 \
  ...
  env NEXT_TELEMETRY_DISABLED=1 NEXT_DIST_DIR="$FRONTEND_DIST_NAME" \
    ...
    NEXT_PUBLIC_EAS_OFFCHAIN_RELAY_URLS="$PUBLIC_RELAYS" \
    NEXT_PUBLIC_EAS_OFFCHAIN_GATEWAYS="$GATEWAYS" \
    pnpm --dir frontend exec next start --hostname 127.0.0.1 --port "$APP_PORT" \
...
EAS_OFFCHAIN_E2E_GATEWAYS="$GATEWAYS" RPC="$RPC" \
  pnpm exec tsx test/e2e/eas-offchain-strict.ts
...
if [ "$BROWSER_MODE" = 1 ]; then
  ...
  run_browser_phase() {
    ...
    env EAS_OFFCHAIN_BROWSER_PHASE="$phase" \
      ...
      node test/e2e/eas-offchain-browser.mjs
  }
