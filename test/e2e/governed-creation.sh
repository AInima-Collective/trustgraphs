...
# The fix is the ensure-by-readback pattern in indexer/src/gov.ts + gov-module-shared.ts; this
...
#   provisioned from test/e2e/params.template.json if absent).
set -euo pipefail
...
[ -f "$PARAMS" ] || cp test/e2e/params.template.json "$PARAMS"
...
forge script script/GovernedCreationE2e.s.sol:CreateGovernedInstanceE2e \
...
ROOT_JSON=$(node --import tsx test/e2e/compute-dev-root.mjs "$BLOB")
...
EXPECTED_CID="$CID" PORT="$STUB_PORT" node test/e2e/kubo-stub.mjs >"$WORK/stub.log" 2>&1 &
STUB_PID=$!
...
forge script script/GovernedCreationE2e.s.sol:SubmitDevRootE2e \
