...
// implementation the indexer's ingestion verifies against (frontend/lib/pagerank/merkle).
// Used by test/e2e/governed-creation.sh to submit a root whose blob the indexer can reproduce.
//
// Usage: node --import tsx test/e2e/compute-dev-root.mjs <blob.json>
// Prints: {"root":"0x…","totalValue":"…"}
import { readFileSync } from 'node:fs'

import * as merkleModule from '../../frontend/lib/pagerank/merkle.ts'

// The frontend package boundary is CommonJS under tsx, so named exports may sit under `default`
// (same normalization as indexer/src/merkle-ingest.ts).
const merkle = 'default' in merkleModule ? merkleModule.default : merkleModule

const blobPath = process.argv[2]
if (!blobPath) {
  console.error('usage: node --import tsx test/e2e/compute-dev-root.mjs <blob.json>')
  process.exit(1)
}
