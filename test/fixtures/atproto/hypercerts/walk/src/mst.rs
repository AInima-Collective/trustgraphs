...
impl<'a> Walker<'a> {
    ...
    pub fn range(car: &'a Car, lo: Vec<u8>, hi: Vec<u8>) -> Self {
        Walker { car, out: vec![], prev_key: None, nodes: 0, node_cids: vec![], lo: Some(lo), hi: Some(hi) }
    }
    ...
    fn walk(&mut self, cid: &Cid, parent_layer: Option<u32>) -> Result<u32, String> {
        ...
        for (i, e) in node.e.iter().enumerate() {
            ...
            if e.p > prev_in_node.len() {
                return Err(format!("{cid}: prefix p={} exceeds prev key len {}", e.p, prev_in_node.len()));
            }
        ...
        for (i, e) in node.e.iter().enumerate() {
            ...
            if let Some(pk) = &self.prev_key {
                if key <= pk {
                    return Err(format!("global key order violation at {}", hex::encode(key)));
                }
            }
