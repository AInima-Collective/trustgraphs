...
// The three DIDs of the regenerated two-sided fixture (tests/fixtures/atproto/hypercerts/fixtures/meta.json).
...
fn load(did: &str, car_rel: &str, plc_rel: &str) -> (B256, AtprotoWitness) {
    ...
    let car = std::fs::read(format!("{root}/tests/fixtures/atproto/hypercerts/fixtures/{car_rel}"))
        .unwrap();
    let plc_json: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(format!(
            "{root}/tests/fixtures/atproto/hypercerts/fixtures/{plc_rel}"
        ))
        .unwrap(),
    )
    .unwrap();
