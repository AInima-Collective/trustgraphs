//! Emit the hypercerts program's canonical golden-vector family (`tests/golden/hypercerts.json`),
//! independently reproduced in Solidity (`contracts/test/unit/golden/HypercertsGoldenVectors.t.sol`) and TS
//! (`packages/frontend/lib/hypercerts/golden.test.ts`) so the four-way byte parity stays enforced.
//!
//! Run: `cargo run -p hypercerts-core --example export_golden > tests/golden/hypercerts.json`
//!
//! The seed DID is read from the fixture's PLC log (so the vectors track whatever repo the
//! `tests/fixtures/atproto/hypercerts` generator currently pins); every hash-formula vector (nodeIds,
...
/// the browser re-derives PageRank + root + journal from it (`packages/frontend/lib/hypercerts/recompute.ts`).
...
fn main() {
    ...
    // `packages/frontend/lib/hypercerts/recompute.ts` re-derives rank → distribute → output_root → blob/cid →
    ...
    let out = json!({
        "note": "generated from tests/fixtures/atproto/hypercerts; seed DID read from the PLC log",
        "seedDid": seed_did,
        "params": {
