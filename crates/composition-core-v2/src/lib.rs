//! Consensus core for the isolated `trust-compose` params/manifest V2 program.
//!
//! V2 extends the V1 final-distribution operator with one closed source
//! compatibility class, so a single composition may blend the standard
//! `trust-graph` and `trust-graph-weighted` allocation outputs without
//! relabelling either source. Every source record carries its real program ID
//! and real output domain, and validation requires the exact reviewed pair.
//! The blend arithmetic, freshness rules, workload caps, output encoding, and
//! journal are deliberately identical to V1. The V1 crate remains the frozen
//! source of the shipped V1 guest and is not imported here, so its build
