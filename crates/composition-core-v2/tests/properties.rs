use composition_core_v2::{fixture::mixed_input, hamilton::apportion};
...
fn allocating_a_source_its_total_reproduces_every_value() {
    ...
    let manifest = composition_core_v2::codec::parse_capture_manifest(&input.manifest, 10).unwrap();
    for (source, preimage) in manifest.sources.iter().zip(&input.source_preimages) {
        let entries =
            composition_core_v2::blob::decode_canonical_score_blob(&preimage.blob).unwrap();
