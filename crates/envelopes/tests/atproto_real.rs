//! Envelope-1 integration tests against REAL fixtures (tests/fixtures/atproto/{repos,hypercerts}):
...
fn real_bluesky_repo_full_pipeline() {
    let (did, head, w) = load_witness(
        "tests/fixtures/atproto/repos/atproto.car",
        "tests/fixtures/atproto/repos/atproto.plc.json",
    );
...
fn legacy_create_genesis_chain_verifies() {
    ...
    let (did, head, w) = load_witness(
        "tests/fixtures/atproto/repos/jay.car",
        "tests/fixtures/atproto/repos/jay.plc.json",
    );
...
fn hypercerts_seeded_repo_verifies_all_collections() {
    let (did, head, w) = load_witness(
        "tests/fixtures/atproto/hypercerts/fixtures/hypercerts.car",
        "tests/fixtures/atproto/hypercerts/fixtures/hypercerts.plc.json",
    );
    let collections = [
    ...
    // Alice's two-sided repo: follow, badge.award, activity, TWO evaluations
