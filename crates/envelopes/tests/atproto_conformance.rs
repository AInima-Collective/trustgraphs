...
//!      vendored under `tests/fixtures/atproto/interop/`:
...
//!
//! See `tests/fixtures/atproto/interop/README.md` for provenance + the Go generator.
...
fn conformance_key_heights() {
    let v = read_json("tests/fixtures/atproto/interop/key_heights.json");
    let mut n = 0;
...
fn conformance_common_prefix() {
    let v = read_json("tests/fixtures/atproto/interop/common_prefix.json");
    let mut n = 0;
    for e in v.as_array().unwrap() {
...
fn conformance_signature_fixtures() {
    let v = read_json("tests/fixtures/atproto/interop/signature-fixtures.json");
...
fn conformance_canonical_trees_walk_exact() {
    for (car_file, want) in [
        ...
        let bytes =
            std::fs::read(fixture(&format!("tests/fixtures/atproto/interop/car/{car_file}")))
                .unwrap();
...
fn adversarial_boundary_fencing() {
    let bytes =
        std::fs::read(fixture("tests/fixtures/atproto/interop/car/two_deep_split.car")).unwrap();
