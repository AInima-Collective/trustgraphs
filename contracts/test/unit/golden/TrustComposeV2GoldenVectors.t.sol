...
contract TrustComposeParamsCodecV2Harness {
    function hash(TrustComposeParamsCodecV2.Params calldata params) external pure returns (bytes32) {
        ...
        return TrustComposeParamsCodecV2.hash(copy);
    }
}

/// @notice Independent Solidity byte lock for `trust-compose` V2 params, capture, output, journal,
///         and the closed source compatibility class.
contract TrustComposeV2GoldenVectorsTest is Test {
    using stdJson for string;

    string internal json;
    TrustComposeParamsCodecV2Harness internal codec;

    function setUp() public {
        json = vm.readFile("tests/golden/trust-compose-v2.json");
        codec = new TrustComposeParamsCodecV2Harness();
    }
    ...
    function test_CompatibilityClassAndAdmittedPairs() public view {
        assertEq(
            TrustComposeValidatorV2.SOURCE_COMPATIBILITY_CLASS, json.readBytes32(".constants.sourceCompatibilityClass")
        );
        assertEq(TrustComposeValidatorV2.SOURCE_COMPATIBILITY_CLASS, TrustComposeValidatorV2.sourceCompatibilityClass());
        assertEq(
            TrustComposeValidatorV2.admittedSourceOutputDomain(TrustComposeValidatorV2.TRUST_GRAPH_PROGRAM_ID),
            json.readBytes32(".constants.admittedPairs[0].sourceOutputDomain")
        );
        assertEq(
            TrustComposeValidatorV2.TRUST_GRAPH_PROGRAM_ID, json.readBytes32(".constants.admittedPairs[0].programId")
        );
        assertEq(
            TrustComposeValidatorV2.admittedSourceOutputDomain(TrustComposeValidatorV2.WEIGHTED_TRUST_GRAPH_PROGRAM_ID),
        ...
        assertEq(TrustComposeValidatorV2.admittedSourceOutputDomain(keccak256("trust-compose")), bytes32(0));
        assertEq(TrustComposeValidatorV2.admittedSourceOutputDomain(keccak256("contributions")), bytes32(0));
    }
    ...
    function test_PolicyManifestLeavesRootAndDigest() public view {
        ...
        assertEq(_readUint(manifest, 4, 2), 2);
    ...
    function test_CapturedManifestCarriesRealProgramsAndDomains() public view {
        ...
        assertEq(bytes32(_readUint(manifest, 23 + 84, 32)), TrustComposeValidatorV2.TRUST_GRAPH_PROGRAM_ID);
        assertEq(bytes32(_readUint(manifest, 23 + 116, 32)), TrustComposeValidatorV2.TRUST_GRAPH_OUTPUT_DOMAIN);
        assertEq(
            bytes32(_readUint(manifest, 23 + 293 + 84, 32)), TrustComposeValidatorV2.WEIGHTED_TRUST_GRAPH_PROGRAM_ID
        );
        assertEq(
            bytes32(_readUint(manifest, 23 + 293 + 116, 32)), TrustComposeValidatorV2.WEIGHTED_TRUST_GRAPH_OUTPUT_DOMAIN
        );
    }
    ...
    function _params() private view returns (TrustComposeParamsCodecV2.Params memory p) {
