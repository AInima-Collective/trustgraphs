...
import {CompositionPolicyV2TestLib} from "../../helpers/CompositionPolicyV2TestLib.sol";
import {MockSafeOwner} from "../../helpers/MockSafeOwner.sol";

contract DirectComposeV2Verifier is IZkVerifier {
...
contract DirectNoVkeyV2Verifier is IZkVerifier {
    function verify(bytes calldata, bytes32) external pure {}
}

contract TrustComposeFactoryV2Test is Test {
    bytes32 internal constant VKEY = keccak256("composition v2 vkey");
    ...
    DirectComposeV2Verifier internal verifier;
    MerkleSnapshotDeployer internal snapshotDeployer;
    MerkleFundDistributorDeployer internal distributorDeployer;
    CompositionSourceAccumulatorV2Deployer internal accumulatorDeployer;
    TrustComposeParamsControllerV2Deployer internal controllerDeployer;
    ...
    function setUp() public {
        ...
        verifier = new DirectComposeV2Verifier(VKEY);
        snapshotDeployer = new MerkleSnapshotDeployer();
        distributorDeployer = new MerkleFundDistributorDeployer();
        accumulatorDeployer = new CompositionSourceAccumulatorV2Deployer();
        controllerDeployer = new TrustComposeParamsControllerV2Deployer();
    ...
    ) internal returns (TrustComposeFactoryV2) {
        return new TrustComposeFactoryV2(
            verifier_,
            vkey,
            registry_,
            adapterFactory_,
    ...
    function test_ConstructorPinsDependenciesAndTimingConstants() public view {
        ...
        assertEq(factory.PROGRAM(), keccak256("trust-compose"), "V2 registers the shared composition program");
    }
    ...
    function test_ConstructorRejectsZeroInvalidAndMismatchedVerifier() public {
        ...
        DirectNoVkeyV2Verifier invalidVerifier = new DirectNoVkeyV2Verifier();
        vm.expectRevert(TrustComposeFactoryV2.InvalidCompositionVerifier.selector);
        ...
        vm.expectRevert(abi.encodeWithSelector(TrustComposeFactoryV2.ProgramVKeyMismatch.selector, supplied, VKEY));
        _deploy(verifier, supplied, registry, adapterFactory, EPOCH_FLOOR, ACTIVATION_DELAY);

        vm.expectRevert(TrustComposeFactoryV2.ZeroEpochFloor.selector);
        _deploy(verifier, VKEY, registry, adapterFactory, 0, ACTIVATION_DELAY);

        vm.expectRevert(TrustComposeFactoryV2.ZeroActivationDelay.selector);
    ...
    function test_ConstructorRejectsAdapterFactoryFromAnotherRegistry() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                TrustComposeFactoryV2.SourceAdapterRegistryMismatch.selector,
                address(registry),
    ...
    function test_CreateInstanceRejectsPaymentWithoutVaultAndNameBoundariesBeforeDeployment() public {
        ...
        vm.expectRevert(TrustComposeFactoryV2.NoVaultConfigured.selector);
        factory.createInstance{value: 1}(args);

        args.name = "";
        vm.expectRevert(TrustComposeFactoryV2.EmptyName.selector);
        factory.createInstance(args);

        args.name = new string(factory.MAX_NAME_BYTES() + 1);
        vm.expectRevert(abi.encodeWithSelector(TrustComposeFactoryV2.NameTooLong.selector, bytes(args.name).length));
        factory.createInstance(args);
    }
    ...
    function test_DistributorCreationAndAttachmentRejectEoaOwners() public {
        ...
        args.params = CompositionPolicyV2TestLib.creationParams(1_000);
        args.policyManifest = CompositionPolicyV2TestLib.manifest(address(0x100), address(0x200), 1_000, false);
