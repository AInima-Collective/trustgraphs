...
import {GovernedTrustComposeFactoryV2} from "src/factory/GovernedTrustComposeFactoryV2.sol";
...
contract ComposeV2ProgramVerifier is IZkVerifier {
...
/// @notice The governed wrapper over the V2 factory. The shared GovernedFactoryBase mechanics are
///         exercised in full by the V1 battery; this suite proves the V2 wrapper wires a MIXED
///         standard+weighted composition to a Safe authority from genesis.
contract GovernedTrustComposeFactoryV2Test is Test {
    ...
    bytes32 internal constant SIGNER_VKEY = keccak256("compose v2 factory signer guest");
    ...
    ComposeV2ProgramVerifier internal sourceVerifier;
    ComposeV2ProgramVerifier internal composeVerifier;
    ComposeV2ProgramVerifier internal signerVerifier;
    TrustComposeFactoryV2 internal factory;
    GovernedTrustComposeFactoryV2 internal governedFactory;
    ...
    function setUp() public {
        ...
        sourceVerifier = new ComposeV2ProgramVerifier(SOURCE_VKEY);
        composeVerifier = new ComposeV2ProgramVerifier(COMPOSE_VKEY);
        signerVerifier = new ComposeV2ProgramVerifier(SIGNER_VKEY);
        ...
        governedFactory = new GovernedTrustComposeFactoryV2(
            factory,
            safeFactory,
    ...
    function test_CreateGovernedMixedInstanceMakesSafeTheAuthorityFromGenesis() public {
        TrustComposeFactoryV2.CreateArgs memory args = _args("member-owned mixed composition");
        ...
        assertEq(TrustComposeParamsControllerV2(controller).owner(), safe, "Safe must own the policy controller");
        ...
        for (uint256 i; i < logs.length; ++i) {
            ...
            if (
                entry.emitter != address(factory) || entry.topics.length != 4
                    || entry.topics[0] != TrustComposeFactoryV2.TrustComposeInstanceCreated.selector
            ...
            (,,,, distributor,,,,,) = abi.decode(
                ...
                (
                    ...
                    bytes32,
                    bytes32,
                    TrustComposeParamsCodecV2.Params
                )
            );
    ...
    function _args(string memory name) internal view returns (TrustComposeFactoryV2.CreateArgs memory args) {
        args.name = name;
        args.metadataURI = "ipfs://governed-composition-v2-metadata";
