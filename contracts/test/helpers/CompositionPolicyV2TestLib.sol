...
/// @notice Canonical mixed two-source V2 policy fixtures shared by direct composition contract
///         tests: source 1 is a standard `trust-graph` output and source 2 a
///         `trust-graph-weighted` output.
library CompositionPolicyV2TestLib {
    uint64 internal constant SCALE = 1e18;

    function standardProgram() internal pure returns (bytes32) {
        return TrustComposeValidatorV2.TRUST_GRAPH_PROGRAM_ID;
    }

    function weightedProgram() internal pure returns (bytes32) {
        return TrustComposeValidatorV2.WEIGHTED_TRUST_GRAPH_PROGRAM_ID;
    }

    function standardDomain() internal pure returns (bytes32) {
        return TrustComposeValidatorV2.TRUST_GRAPH_OUTPUT_DOMAIN;
    }

    function weightedDomain() internal pure returns (bytes32) {
        return TrustComposeValidatorV2.WEIGHTED_TRUST_GRAPH_OUTPUT_DOMAIN;
    }
    ...
    function creationParams(uint64 maxAge) internal pure returns (TrustComposeParamsCodecV2.Params memory p) {
        p.version = 2;
    ...
    function finalParams(
        ...
        uint64 maxAge,
        bool rotated
    ) internal view returns (TrustComposeParamsCodecV2.Params memory p) {
        ...
        returns (bytes memory encoded)
    {
        ...
        encoded = abi.encodePacked(bytes4("TGCP"), uint16(2), uint64(block.chainid), uint8(2));
