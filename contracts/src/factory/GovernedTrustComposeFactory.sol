...
/// @notice The governed wrapper around `TrustComposeFactory`: one transaction creates
///         a trust-compose network and a module-only DAO Safe with member governance, delayed
...
contract GovernedTrustComposeFactory is GovernedFactoryBase {
    ...
    /// @notice Create and atomically link a DAO-governed composition beneath a parent network.
    function createGovernedSubnetwork(
        ...
        return _installGovernedSubnetwork(
            ...
            requested.name,
            requested.salt,
            abi.encodeCall(TrustComposeFactory.createInstance, (args)),
            policy,
            signerSync,
            parentInstanceId,
            tier
    ...
    /// @notice Create one DAO-governed trust-compose network. `requested.admin` is deliberately
