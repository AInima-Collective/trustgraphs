...
/// @notice Creates a weighted-prior trust graph and a module-only DAO Safe as one transaction —
///         the `GovernedTrustgraphsFactory` shape applied to the `trust-graph-weighted` program.
/// @dev The base `WeightedTrustgraphsFactory` remains the canonical instance creator and the only
///      holder of `REGISTRAR_ROLE`; this wrapper temporarily owns a fresh one-owner Safe, has that
///      Safe call `createInstance` (so the Safe is the creator AND the admin), installs the
...
///      Emits the SAME `GovernedInstanceCreated`/`GovernedAuthorityInstalled` signatures as every
///      governed wrapper, so the indexer registers one handler N times. Signer-sync is plumbed
///      (the deployer fails closed on a program-vkey mismatch) but the only signer guest today
///      proves the trust-graph selection pipeline, so the app does not offer it here.
contract GovernedWeightedTrustgraphsFactory {
    ...
    /// @notice Optional score-selected Safe owner rotation. The wrapper pins the verifier/vkey at
    ///         deployment; no weighted signer guest exists yet, so the app does not offer this path.
    ...
    WeightedTrustgraphsFactory public immutable FACTORY;
    ...
    /// @notice Create one DAO-governed weighted-prior trust graph. `requested.admin` is
    ///         deliberately ignored: the newly-created Safe is the instance admin, controller
    ///         owner, and fund owner.
    function createGovernedInstance(
        ...
        if (msg.value == 0) {
            ...
            // The vault decides the newborn band (clarification: never a hardcoded literal —
            // flat-banded programs start above band 1, unknown programs answer 0 = unpriced).
