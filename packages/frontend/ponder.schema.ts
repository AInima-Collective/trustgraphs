 ...
 *
 * Everything a network page needs to render is carried here (packages/frontend/lib/types.ts `Network`):
 ...
 * the same TS port the browser recompute uses (packages/frontend/lib/pagerank/encode) — it always equals
...
export const signerSyncModule = onchainTable(
  ...
  (t) => ({
    ...
    maxInactiveBlocks: t.bigint().notNull(),
    minActivityWitnesses: t.integer().notNull(),
