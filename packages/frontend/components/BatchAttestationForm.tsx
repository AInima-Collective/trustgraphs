...
import type { Network, NetworkEntry } from '@/lib/types'
...
export function BatchAttestationForm({
  network,
  schemaUid,
  accountData = [],
  attestationsData = [],
  ...
  attestationsData?: AttestationData[]
  ...
  const vouchSchema = network.schemas.find(
    (schema) => schema.key === 'vouching' && schema.uid === schemaUid
  )
  ...
  useEffect(() => {
    onBusyChange(isCreating || isResolving)
    return () => onBusyChange(false)
  }, [isCreating, isResolving, onBusyChange])

  const close = () => {
    if (!isCreating && !isResolving) onClose()
  }
    ...
    <div>
      <div className="border-b border-hairline px-5 py-5 sm:px-7 sm:py-6">
        ...
        <h3 className="tg-display mt-2 max-w-2xl text-2xl text-text sm:text-3xl">
          Attest to several accounts at once.
        </h3>
        <p className="mt-3 max-w-2xl text-sm text-text-muted">
          Each recipient gets a separate confidence score and revocable vouch.
          The batch is submitted together and either succeeds together or not at
          all.
        </p>
      </div>
        ...
        <div className="p-5 sm:p-7">
          <div className="grid gap-7 lg:grid-cols-[minmax(0,1fr)_20rem]">
            <section>
              ...
              <p className="mt-3 max-w-2xl text-sm text-text-muted">
                Check every destination. A new vouch replaces your current vouch
                for that recipient; older history remains visible.
              </p>
          ...
          <div className="mt-7 flex flex-col-reverse gap-3 border-t border-hairline pt-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Button
              ...
              size="lg"
              onClick={() => void submit()}
              disabled={!isConnected || isCreating || isResolving}
            >
      ...
      {steps.map((label, index) => {
        ...
        return (
          <li
            key={label}
            className={cn(
              'flex flex-wrap items-center gap-x-2 gap-y-1 border-r border-hairline px-2 py-2.5 text-[10px] uppercase tracking-wider last:border-r-0 sm:px-5',
