...
assert.equal(actions[0]?.contractName, 'SignerSyncZkModule')
assert.equal(actions[0]?.functionSignature, 'setParamsHash(bytes32)')
assert.equal(actions[1]?.target, address('5'))
assert.equal(actions[1]?.contractName, 'TrustgraphsParamsController')
assert.equal(actions[1]?.functionSignature, 'updateParams(Params,string)')
