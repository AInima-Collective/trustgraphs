...
- `config.typecheck.json` is a deliberately address-free local compile template used only when a
  clean checkout has no generated development config. `prepare:typecheck-config` links ignored
