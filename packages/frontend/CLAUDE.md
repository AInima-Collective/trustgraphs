...
│   ├── schema-components/ # Schema-specific form components
│   └── *.tsx             # UI primitives and feature components
...
- `NEXT_PUBLIC_WEBSOCKET_URL_11155111` - Optional Sepolia WebSocket RPC
- `NEXT_PUBLIC_IPFS_GATEWAY` - IPFS gateway URL
- `NODE_ENV` - Controls which config.{env}.json is used

### Config System

- **config.json**: Symlink to generated `config.development.json` or `config.sepolia.json`
...
- `config.typecheck.json` is a minimal local compile template used only when a clean checkout has
  no generated development config. It includes the canonical public Sepolia EAS address because
  generated bindings export `easAddress`, which application modules import during typechecking;
  every project-owned contract remains unset. `prepare:typecheck-config` links ignored
  `config.json`/`networks.json` inputs for code generation; it never represents a deployment.
...
1. **Use shadcn/ui components** from `components/` for UI primitives (Button, Card, Input, etc.)
...
const { data, isLoading } = useQuery({
  ...
  queryFn: () =>
    ponderQuery.findMany.attestation({
...
3. Create encoding/decoding helpers in `lib/` or hook
4. Optionally add a custom form component in `components/schema-components/`
...
2. Deploy contracts (`bash ./contracts/script/deploy-script.sh` in root)
