...
pub fn parse_multikey(did_key: &str) -> Result<Multikey, String> {
    ...
    let raw = bs58::decode(s).into_vec().map_err(|e| format!("base58 decode: {e}"))?;
...
pub fn decode_commit(block: &[u8]) -> Result<CommitFields, String> {
    let val: Ipld =
        serde_ipld_dagcbor::from_slice(block).map_err(|e| format!("commit decode: {e}"))?;
    let map = match val {
        Ipld::Map(m) => m,
...
pub fn verify_commit_sig(mk: &Multikey, unsigned: &[u8], sig: &[u8]) -> Result<(), String> {
    ...
    match mk.curve {
        Curve::K256 => {
            ...
            let vk =
                VerifyingKey::from_sec1_bytes(&mk.key).map_err(|e| format!("k256 key: {e}"))?;
            ...
            vk.verify_prehash(&digest, &s).map_err(|e| format!("k256 verify: {e}"))
        }
        Curve::P256 => {
            use p256::ecdsa::signature::hazmat::PrehashVerifier;
            use p256::ecdsa::{Signature, VerifyingKey};
            let vk =
                VerifyingKey::from_sec1_bytes(&mk.key).map_err(|e| format!("p256 key: {e}"))?;
            let s = Signature::from_slice(sig).map_err(|e| format!("p256 sig: {e}"))?;
            if s.normalize_s().is_some() {
                return Err("p256 signature is high-S (malleable) — rejected".into());
            }
            vk.verify_prehash(&digest, &s).map_err(|e| format!("p256 verify: {e}"))
        }
    }
}
...
mod tests {
    ...
    fn multikey_curves() {
        ...
        let mk =
            parse_multikey("did:key:zQ3shunBKsXixLxKtC5qeSG9E4J5RkGN57im31pcTzbNQnm5w").unwrap();
