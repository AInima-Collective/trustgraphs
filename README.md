...
Participants attest to each other directly against [EAS](https://attest.org); a
**Trust-Aware PageRank** over that graph — seeded by a curated set of trusted accounts so
Sybil rings stay isolated — produces each account's score. New here: the scores are not
computed by a server or an operator committee. Anyone can compute them and prove the
computation correct with an **SP1 zero-knowledge proof**; the chain verifies the proof and
commits the `{account → score}` merkle root. Governance, reward distribution, and even a
Safe multisig's owner set consume that proven root.

The same machinery generalizes past vouching: TrustGraph is a **platform of ZK-proven
...
one canonical Rust core per program, compiled into the SP1 guest, cross-checked
byte-for-byte against Solidity and TypeScript ports, and verified on-chain against inputs
the chain itself accumulated (so provers can't omit or invent attestations).
...
[`docs/PROGRAMS.md`](./docs/PROGRAMS.md) is the authoritative index (vkeys, instances, docs):

| Program | What it proves | Status | Docs |
|---|---|---|---|
| **trust-graph** | the `{account → score}` root over the EAS vouch graph | **Live** | [architecture](./docs/trust-graph/ARCHITECTURE.md) · [runbook](./docs/trust-graph/RUNBOOK.md) · [local testing](./docs/trust-graph/LOCAL_TESTING.md) |
...
checkpoint, reconstructs the prover's input from chain with `input-exporter` (self-checking
that it re-folds to the on-chain accumulator), and cross-checks the SP1 guest against the
native implementation — printing `E2E PASS`.

Needs [Foundry](https://getfoundry.sh) (`anvil`/`forge`/`cast`), Rust (`cargo`), `jq`,
Docker, Node 21+ with pnpm, [go-task](https://taskfile.dev), and the SP1 toolchain — install
walkthrough in [`docs/SETUP.md`](./docs/SETUP.md). The first run builds the guest ELF, so
give it a few minutes; after that it's seconds.
...
[trust-graph runbook](./docs/trust-graph/RUNBOOK.md) (and §8 of the local-testing guide).
...
|---|---|
| `src/contracts/` | Solidity: EAS resolvers + attestation accumulator, `MerkleSnapshot`, SP1 journal verifier, governance/reward/Zodiac modules |
...
Cross-language parity is enforced per program: `test/golden/<program>.json` locks the native
Rust, SP1 guest, Solidity, and TypeScript implementations to identical bytes
(`task zk:parity PROGRAM=<name>`).
