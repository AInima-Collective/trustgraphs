...
**guest change and a verification-key rotation** on our side — a governance event with lead time,
not a dependency bump. **Ask:** release-notes lead time (2 weeks is comfortable) for any change
...
lexicon), so records land unvalidated and our in-guest deterministic skip rules are the only shape
